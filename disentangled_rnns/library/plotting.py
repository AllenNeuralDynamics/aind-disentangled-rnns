...
def plot_bottlenecks(
  ...
  if isinstance(disrnn_config, multisubject_disrnn.MultisubjectDisRnnConfig):
    ...
    update_subj_sigmas_t = np.transpose(
      disrnn.reparameterize_sigma(params_disrnn["update_net_subj_sigma_params"])
    )
...
def plot_choice_rule(
  ...
  if n_latents_to_plot == 1:
    ...
    ax.tick_params(axis="both", labelsize=small)
    output[
      "policy_latent_{}_vals".format(policy_latent_idx_in_latent_space + 1)
    ] = policy_latent_vals
    ...
    output["yhats"] = y_hats
    output[
      "policy_latent_{}_vals".format(policy_latent_idx1_in_latent_space + 1)
    ] = latent0_vals
    output[
      "policy_latent_{}_vals".format(policy_latent_idx2_in_latent_space + 1)
    ] = latent1_vals
